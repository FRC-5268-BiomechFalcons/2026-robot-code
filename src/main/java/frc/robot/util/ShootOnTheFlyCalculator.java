...
public class ShootOnTheFlyCalculator {
    ...
    public record ShooterCommand(Rotation2d robotHeading, double rpm, double effectiveDistance) {
    }
    ...
    public ShootOnTheFlyCalculator() {
        this.shooterTable = new InterpolatingTreeMap<>(InverseInterpolator.forDouble(),
            (start, end, t) -> new ShooterParams(MathUtil.interpolate(start.rpm(), end.rpm(), t),
     ...
     */
    public ShooterCommand calculateStationary(Translation2d robotPosition, Translation2d goalPosition) {
     ...
     * @return Shooter command with turret angle, RPM, and hood angle
     */
    public ShooterCommand calculate(Translation2d robotPosition, Translation2d robotVelocity,
            Translation2d goalPosition, double latencyCompensation) {
        ...
        Rotation2d robotHeading = shotVelocity.getAngle();
        ...
        return new ShooterCommand(robotHeading, adjusted.rpm(), adjusted.effectiveDistance());
    }
     ...
     */
    private ShooterCommand calculateBothAdjustments(double distance, ShooterParams baseline,
            double requiredVelocity) {

        /*
         * baseline is how fast you need to shoot given 0 velocity robot movement requiredVelocity
         * is how fast the ball needs to exit your shooter when accounting for robot velocity
         * 
         * Tof(x) is map from distance -> time traveled. Vel(distance) = distance / Tof(distance) =
         * velocity
         * 
         * we'll use newton's method to figure out wwhat distance input generates our desired
         * velocity. we can look for the roots of f(x) = Vel(x) - requiredVelocity to get us our
         * shooter params, where Vel(x) = x/Tof(x)
         * 
         * x_n+1 = x_n - f(x_n) / f'(x_n)
         * 
         * f(x_n) = currentVelocity - requiredVelocity f'(x_n) = Vel'(x)
         * 
         */
        ...
        for (int i = 0; i < 10 && Math.abs(currentVelocity - requiredVelocity) > 0.005; i++) {
            ...
            double lowVel = (currentDistance - EPSILON) /
                shooterTable.get(currentDistance - EPSILON).timeOfFlight();
            double highVel = (currentDistance + EPSILON) /
                shooterTable.get(currentDistance + EPSILON).timeOfFlight();
        ...
        return new ShooterCommand(null, currentParams.rpm(), currentDistance);
    }
}